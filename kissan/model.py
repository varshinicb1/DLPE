...
class KissanGPT(nn.Module):
    """ The Kissan Neural Network"""

    def __init__(self, vocab_size=None):
        super().__init__()
        # Use provided vocab_size or default from config
        v_size = vocab_size if vocab_size is not None else KissanConfig.vocab_size
        
        # each token directly reads off the logits for the next token from a lookup table
        self.token_embedding_table = nn.Embedding(v_size, KissanConfig.n_embd)
        self.position_embedding_table = nn.Embedding(KissanConfig.block_size, KissanConfig.n_embd)
        self.blocks = nn.Sequential(*[Block(KissanConfig.n_embd, KissanConfig.n_head) for _ in range(KissanConfig.n_layer)])
        self.ln_f = nn.LayerNorm(KissanConfig.n_embd) # final layer norm
        self.lm_head = nn.Linear(KissanConfig.n_embd, v_size)
    ...
    def forward(self, idx, targets=None):
        ...
        pos_emb = self.position_embedding_table(torch.arange(T, device=KissanConfig.device)) # (T,C)
